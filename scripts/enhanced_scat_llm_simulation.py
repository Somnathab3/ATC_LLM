...
def generate_enhanced_llm_prompt(ownship_state: AircraftState, intruder_states: List[AircraftState], 
    ...
    for wpt_id, lat, lon, dist in nearby_waypoints[:5]:  # Top 5 nearest
        ...
        waypoint_options.append(f"{wpt_id}: {dist:.1f}NM at {bearing:.0f}°")
...
Current position: {ownship_state.latitude:.4f}°N, {ownship_state.longitude:.4f}°E
Current heading: {ownship_state.heading_deg:.0f}°
...
- Distance: {conflict['distance_nm']:.1f} NM at {conflict['relative_bearing']:.0f}° relative bearing
- Altitude: {conflict['altitude_ft']:.0f} feet  
- Heading: {conflict['heading_deg']:.0f}°, Speed: {conflict['speed_kt']:.0f} knots
...
1. TURN: Change heading by specified degrees
   - "turn_left X" or "turn_right X" where X is degrees (max 45°)
...
def main():
    ...
    try:
        ...
        log.info(f"✓ Loaded {len(states)} states, created route with {len(route_waypoints)} waypoints")
        ...
        log.info(f"✓ Navigation initialized: start {start_pos}, heading {initial_heading:.1f}°")
        ...
        for cycle in range(SIMULATION_CYCLES):
            ...
            for intruder in intruder_states:
                ...
                if distance < CONFLICT_THRESHOLD_NM:
                    ...
                    log.info(f"🚨 CONFLICT DETECTED: {intruder.aircraft_id} at {distance:.2f}NM")
                    ...
                    if resolution:
                        ...
                        if resolution.resolution_type == ResolutionType.HEADING_CHANGE:
                            ...
                            log.info(f"✈️ Heading changed: {nav_state.heading_deg:.1f}° → {resolution.new_heading_deg:.1f}°")
            ...
            if nav_state.target_waypoint:
                ...
                log.info(f"Cycle {cycle}: Pos=({nav_state.position[0]:.4f},{nav_state.position[1]:.4f}), "
                        f"Hdg={nav_state.heading_deg:.1f}°, Target dist={distance_to_target:.2f}NM")
                
                if nav_state.llm_modified_heading is not None:
                    log.info(f"  🤖 Following LLM heading: {nav_state.llm_modified_heading:.1f}° "
        ...
        log.info(f"🔄 Cycles: {metrics['simulation_cycles']}")
        log.info(f"📍 Waypoints: {metrics['waypoints_reached']}/{metrics['total_waypoints']}")
        log.info(f"✈️  Distance: {metrics['distance_traveled_nm']:.2f} NM")
        log.info(f"⚠️  Conflicts: {metrics['conflicts_detected']}")
        log.info(f"🤖 LLM Resolutions: {metrics['successful_llm_resolutions']}")
        log.info(f"🎯 Heading Changes: {len(metrics['heading_changes'])}")
