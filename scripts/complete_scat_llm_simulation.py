        ...
        detect_output = {
            ...
            "scenario": {
                "ownship": {
                    ...
                    "position": f"{ownship_state.latitude:.4f}°N, {ownship_state.longitude:.4f}°E",
                    "altitude": f"{ownship_state.altitude_ft:.0f} feet",
                    "heading": f"{ownship_state.heading_deg:.0f}°",
    ...
    try:
        ...
        for cycle in range(SIMULATION_CYCLES):
            ...
            if intruder_pos:
                ...
                if conflict:
                    log.info(f"🚨 CONFLICT DETECTED: {conflict.intruder_id} at {conflict.distance_at_cpa_nm:.2f}NM in {conflict.time_to_cpa_min:.1f}min")
                    ...
                    if resolution:
                        log.info(f"🤖 LLM RESOLUTION: {resolution.resolution_type.value}")
                        ...
                        log.warning("❌ LLM resolution failed")
        ...
        log.info(f"⏱️  Duration: {metrics['duration_seconds']:.1f} seconds")
        ...
        log.info(f"✈️  Distance: {metrics['distance_traveled_nm']:.2f} NM")
        log.info(f"⚠️  Conflicts: {metrics['conflicts_detected']}")
        log.info(f"🤖 LLM Queries: {metrics['llm_queries']}")
        log.info(f"✅ Successful Resolutions: {metrics['successful_llm_resolutions']}")
        log.info(f"❌ Failed Resolutions: {metrics['failed_llm_resolutions']}")
        log.info(f"📊 LLM Success Rate: {metrics['llm_performance']['success_rate']:.1f}%")
        ...
        json_file = f"scat_llm_simulation_complete_{timestamp}.json"
        with open(json_file, 'w') as f:
            json.dump(metrics, f, indent=2)
        log.info(f"📄 Complete results saved to {json_file}")
        
        # Save performance summary CSV
        csv_file = f"scat_llm_performance_summary_{timestamp}.csv"
        ...
        log.info(f"📊 Performance summary saved to {csv_file}")
        ...
        plot_file = f"scat_llm_plots_{timestamp}.json"
        ...
        log.info("\\n📋 === FINAL SUMMARY REPORT ===")
        log.info(f"✅ Successfully completed all 5 requirements:")
        log.info(f"   1. ✓ Loaded SCAT file: {SCAT_FILE} ({len(states)} states)")
        log.info(f"   2. ✓ Initialized BlueSky and LLM (llama3.1:8b)")
        log.info(f"   3. ✓ Ran {metrics['simulation_cycles']} simulation cycles with {metrics['llm_queries']} LLM queries")
        log.info(f"   4. ✓ Collected comprehensive metrics in JSON/CSV format")
        log.info(f"   5. ✓ Generated performance summaries and plot data")
        log.info(f"")
        ...
        try:
            ...
            final_file = f"scat_llm_final_metrics_{final_timestamp}.json"
