...
This script fin            'ö': 'o',            # O with diaeresis
            '⚠': '[WARN]',       # Warning sign
        } and replaces Unicode characters (specifically emojis) in Python files
...
class UnicodePatternCleaner:
    ...
    def __init__(self):
        ...
        self.unicode_mappings = {
            ...
            'pi': 'pi',         # Pi
            'phi': 'phi',        # Phi (latitude)
            'lambda': 'lambda',     # Lambda (longitude)
            'delta': 'delta',      # Delta (change/difference)
            '+/-': '+/-',        # Plus-minus
            '~=': '~=',         # Approximately equal
            '<=': '<=',         # Less than or equal
            ...
            'x': 'x',          # Multiplication
            'theta': 'theta',      # Theta
            'delta_small': 'delta_small', # Delta lowercase
