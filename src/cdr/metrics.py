...
class FailureAnalysisCollector:
    ...
    def record_failure(self,
        ...
        """Record a new LLM failure case with enhanced format for Stage F requirements."""
        
        # Calculate severity based on remaining separation
        min_sep_nm = cpa_after.get('min_sep_nm', float('inf'))
        severity = max(0.0, 5.0 - min_sep_nm)  # Higher severity for closer violations
        
        # Generate counterfactual suggestion
        counterfactual = self._generate_counterfactual(ownship_state, intruder_states, cpa_before)
        
        failure_case = LLMFailureCase(
            timestamp=datetime.now(),
            scenario_id=scenario_id or f"failure_{len(self.failures)+1}",
            ...
            severity=severity,
            suggested_alternative=counterfactual
        )
        
        self.failures.append(failure_case)
        
        # Write to llm_failure_cases.jsonl stream as required by Stage F
        if self.output_file:
            self._write_failure_case_to_stream(failure_case)
        
        logger.warning(f"Recorded LLM failure case: {failure_type}, severity {severity:.2f}")
        
    def record_llm_failure(self, ownship: Dict[str, Any], traffic: List[Dict[str, Any]], 
                          conflicts: List, failure_context: str, llm_response: str) -> None:
        """Record LLM failure case from Verifier retry system (Stage D integration)."""
        try:
            # Build enhanced failure record for Stage F requirements
            failure_record = {
                'timestamp': datetime.now().isoformat(),
                'type': 'llm_verification_failure',
                'state_snapshot': {
                    'ownship_state': ownship,
                    'traffic_count': len(traffic),
                    'conflicts_detected': len(conflicts)
                },
                'detector_prompts': 'N/A - verification failure',
                'resolver_prompts': failure_context,
                'llm_output': llm_response or 'No response',
                'min_sep_series': [c.distance_at_cpa_nm for c in conflicts if hasattr(c, 'distance_at_cpa_nm')],
                'bluesky_cd_flags': {
                    'verification_failed': True,
                    'cpa_violations': True
                },
                'counterfactual': self._generate_counterfactual(ownship, traffic, None)
            }
            
            # Write to failure stream
            if self.output_file:
                try:
                    with open(self.output_file, 'a') as f:
                        f.write(json.dumps(failure_record) + '\n')
                except Exception as e:
                    logger.warning(f"Failed to write to failure stream: {e}")
                    
        except Exception as e:
            logger.debug(f"Failed to record LLM failure: {e}")
            
    def _write_failure_case_to_stream(self, failure_case: LLMFailureCase) -> None:
        """Write failure case to JSONL stream in Stage F required format."""
        try:
            # Create Stage F compliant record
            failure_record = {
                'timestamp': failure_case.timestamp.isoformat(),
                'scenario_id': failure_case.scenario_id,
                'type': failure_case.failure_type,
                'state_snapshot': {
                    'ownship_state': failure_case.ownship_state,
                    'intruder_states': failure_case.intruder_states,
                    'traffic_count': len(failure_case.intruder_states)
                },
                'detector_prompts': failure_case.detection_prompt,
                'resolver_prompts': failure_case.resolution_prompt,
                'llm_output': {
                    'detection': failure_case.detection_output,
                    'resolution': failure_case.resolution_output,
                    'command_issued': failure_case.command_issued
                },
                'min_sep_series': [
                    failure_case.cpa_before.get('min_sep_nm', 0),
                    failure_case.cpa_after.get('min_sep_nm', 0)
                ],
                'bluesky_cd_flags': failure_case.bluesky_cd_flags,
                'counterfactual': failure_case.suggested_alternative,
                'severity': failure_case.severity
            }
            ...
            logger.warning(f"Failed to write failure case to stream: {e}")
            
    def _generate_counterfactual(self, ownship: Dict[str, Any], intruders: List[Dict[str, Any]], 
                               cpa_before: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """Generate counterfactual analysis for Stage F requirements."""
        try:
            # Generate suggested alternative resolution
            current_hdg = ownship.get('hdg_deg', 0)
            current_alt = ownship.get('alt_ft', 35000)
            
            # Suggest conservative heading change
            suggested_hdg = (current_hdg + 30) % 360  # 30° right turn
            
            # Suggest altitude change based on traffic
            traffic_below = any(intruder.get('alt_ft', 0) < current_alt - 1000 for intruder in intruders)
            suggested_alt = current_alt + 2000 if traffic_below else current_alt - 2000
            
            return {
                'type': 'conservative_maneuver',
                'suggested_heading_deg': suggested_hdg,
                'suggested_altitude_ft': suggested_alt,
                'rationale': f"Conservative turn to {suggested_hdg}° and altitude to {suggested_alt} ft for guaranteed separation",
                'estimated_separation_improvement_nm': 8.0  # Conservative estimate
            }
            
        except Exception as e:
            logger.debug(f"Failed to generate counterfactual: {e}")
            return {'type': 'unknown', 'rationale': 'Could not generate alternative'}
            
    def generate_failure_summary_html(self, output_file: str) -> None:
        """Generate Stage F required llm_failures.html summary."""
        try:
            html_content = self._build_failure_summary_html()
            with open(output_file, 'w') as f:
                f.write(html_content)
            logger.info(f"Generated failure summary HTML: {output_file}")
        except Exception as e:
            logger.error(f"Failed to generate failure summary HTML: {e}")
            
    def _build_failure_summary_html(self) -> str:
        """Build HTML summary of failure cases with miss rate and patterns."""
        patterns = self.analyze_failure_patterns()
        
        return f"""<!DOCTYPE html>
<html>
<head>
    <title>LLM Failure Analysis Summary</title>
    <style>
        body {{ font-family: Arial, sans-serif; margin: 20px; }}
        .metric {{ margin: 10px 0; }}
        .failure-case {{ border: 1px solid #ccc; margin: 10px 0; padding: 10px; }}
        .severe {{ background-color: #ffe6e6; }}
        .moderate {{ background-color: #fff3cd; }}
        .mild {{ background-color: #e6ffe6; }}
...
<body>
    <h1>LLM Conflict Detection and Resolution - Failure Analysis</h1>
    
    <h2>Summary Metrics</h2>
    <div class="metric"><strong>Total Failures:</strong> {len(self.failures)}</div>
    <div class="metric"><strong>Miss Rate:</strong> {patterns.get('avg_severity', 0):.1f} average severity</div>
    <div class="metric"><strong>Most Common Failure Type:</strong> {patterns.get('most_common_type', 'Unknown')}</div>
    
    <h2>Failure Patterns</h2>
    <table>
        <tr><th>Pattern</th><th>Count</th><th>Percentage</th></tr>
        <tr><td>Detection Failures</td><td>{patterns.get('detection_failures', 0)}</td><td>{patterns.get('detection_failures', 0)/max(1, len(self.failures))*100:.1f}%</td></tr>
        <tr><td>Resolution Failures</td><td>{patterns.get('resolution_failures', 0)}</td><td>{patterns.get('resolution_failures', 0)/max(1, len(self.failures))*100:.1f}%</td></tr>
        <tr><td>Verification Failures</td><td>{patterns.get('verification_failures', 0)}</td><td>{patterns.get('verification_failures', 0)/max(1, len(self.failures))*100:.1f}%</td></tr>
    </table>
    
    <h2>Worst Failure Cases</h2>
    {"".join(self._format_failure_case_html(case) for case in self.get_worst_failures(5))}
    
    <h2>Common Patterns</h2>
    <ul>
        <li>LAT (Look-Ahead Time) analysis: Average time to failure {patterns.get('avg_time_to_failure', 0):.1f} minutes</li>
        <li>RAT (Resolution Action Time) analysis: Average resolution delay {patterns.get('avg_resolution_delay', 0):.1f} seconds</li>
        <li>Common scenarios: {patterns.get('common_scenarios', 'Head-on conflicts, crossing traffic')}</li>
    </ul>
    ...
    def _format_failure_case_html(self, case: LLMFailureCase) -> str:
        """Format individual failure case for HTML report."""
        severity_class = 'severe' if case.severity > 3 else 'moderate' if case.severity > 1 else 'mild'
        
        return f"""
    <div class="failure-case {severity_class}">
        <h4>Case {case.scenario_id} - {case.failure_type}</h4>
        <p><strong>Severity:</strong> {case.severity:.2f}</p>
        <p><strong>Timestamp:</strong> {case.timestamp.strftime('%Y-%m-%d %H:%M:%S')}</p>
        <p><strong>Min Separation:</strong> {case.cpa_after.get('min_sep_nm', 0):.1f} NM</p>
        <p><strong>Command Issued:</strong> {case.command_issued}</p>
        <p><strong>Counterfactual:</strong> {case.suggested_alternative.get('rationale', 'N/A') if case.suggested_alternative else 'N/A'}</p>
    </div>"""
