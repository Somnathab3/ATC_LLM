...
class RealLLMVerifier:
    ...
    def __init__(self):
        ...
        logger.info("🚀 Real LLM Verifier initialized - NO MOCKING ALLOWED")
    ...
    def _disable_all_mocks(self):
        ...
        logger.info("✅ All mock modes forcibly disabled")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.model_name in model_names:
                    logger.info(f"✅ Target model '{self.model_name}' is available")
                    return True
                else:
                    logger.error(f"❌ Target model '{self.model_name}' not found")
                    return False
            else:
    ...
    def call_ollama_api(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"📥 LLM response received ({response_time:.2f}s)")
    ...
    def test_basic_llm_response(self) -> bool:
        ...
        logger.info("🧠 Testing basic LLM response...")
...
Replace the placeholder values with actual responses. This verifies you are a real LLM."""
        ...
        try:
            ...
            if json_match:
                ...
                if all(field in json_data for field in required_fields):
                    logger.info(f"✅ Real LLM verified: {json_data}")
                    return True
                else:
                    logger.error(f"❌ Response missing required fields: {json_data}")
                    return False
            else:
    ...
    def test_aviation_prompt(self) -> bool:
        ...
        logger.info("✈️ Testing aviation-specific prompt...")
...
AIRCRAFT DATA:
- Ownship: SAS101, A320, Position: 59.6519°N 17.9186°E, Altitude: 35000 ft, Speed: 450 kt, Heading: 090°
- Traffic: DLH456, B737, Position: 59.6600°N 17.9500°E, Altitude: 35000 ft, Speed: 430 kt, Heading: 270°
...
Respond with JSON only:
{
    ...
    "assessment": "detailed explanation"
        ...
        if not result["success"]:
            logger.error("❌ Aviation prompt test failed")
            return False
        ...
        try:
            ...
            if json_match:
                ...
                if all(field in json_data for field in required_fields):
                    logger.info(f"✅ Aviation analysis successful: {json_data}")
                    return True
                else:
                    logger.error(f"❌ Missing aviation fields: {json_data}")
                    return False
            else:
                logger.error("❌ No JSON in aviation response")
                logger.debug(f"Raw response: {response_text}")
                return False
                
        except json.JSONDecodeError as e:
            logger.error(f"❌ Aviation JSON parsing failed: {e}")
            return False
    
    def test_resolution_prompt(self) -> bool:
        """Test conflict resolution prompt."""
        logger.info("🔧 Testing conflict resolution prompt...")
...
- Current heading: 090°, Speed: 450 kt
...
Respond with JSON only:
{
    ...
    "rationale": "explanation of resolution strategy"
        ...
        if not result["success"]:
            logger.error("❌ Resolution prompt test failed")
            return False
            ...
            if json_match:
                ...
                if all(field in json_data for field in required_fields):
                    ...
                    if "SAS101 HDG" in bluesky_cmd:
                        logger.info(f"✅ Resolution successful: {json_data}")
                        return True
                    else:
                        logger.error(f"❌ Invalid BlueSky command: {bluesky_cmd}")
                        return False
                else:
                    logger.error(f"❌ Missing resolution fields: {json_data}")
                    return False
            else:
                logger.error("❌ No JSON in resolution response")
                logger.debug(f"Raw response: {response_text}")
                return False
                
        except json.JSONDecodeError as e:
            logger.error(f"❌ Resolution JSON parsing failed: {e}")
            return False
    
    def load_scat_sample(self, scat_file: str) -> Dict[str, Any]:
        """Load sample SCAT data for testing."""
        logger.info(f"📂 Loading SCAT sample: {scat_file}")
        ...
        try:
            ...
            if not fpl_base or not plots:
                logger.error("❌ Invalid SCAT data structure")
                return {}
            ...
            logger.info(f"✅ SCAT data loaded: {aircraft_state['callsign']}")
            return aircraft_state
            
        except Exception as e:
            logger.error(f"❌ SCAT loading failed: {e}")
            return {}
    
    def test_scat_to_llm_chain(self, scat_file: str) -> bool:
        """Test complete SCAT -> LLM -> BlueSky chain."""
        logger.info("🔄 Testing complete SCAT -> LLM -> BlueSky chain...")
...
2. Generate a BlueSky command to change heading to 270°
...
{{
    ...
    "timestamp": "current_time"
        ...
        if not result["success"]:
            logger.error("❌ SCAT -> LLM chain failed")
            return False
        ...
        try:
            ...
            if json_match:
                ...
                if expected_pattern in bluesky_cmd:
                    logger.info(f"✅ Complete chain successful!")
                    ...
                    logger.error(f"❌ Incorrect BlueSky command: {bluesky_cmd}")
                    return False
            else:
                logger.error("❌ No JSON in chain response")
                return False
                
        except json.JSONDecodeError as e:
            logger.error(f"❌ Chain JSON parsing failed: {e}")
            return False
    
    def run_iterative_test(self, max_iterations: int = 3) -> bool:
        """Test iterative BlueSky <-> LLM communication."""
        logger.info(f"🔄 Testing iterative communication ({max_iterations} rounds)...")
...
- Position: {aircraft['position']['lat']:.4f}°N {aircraft['position']['lon']:.4f}°E
    ...
    "reason": "brief explanation"
            ...
            try:
                ...
                if json_match:
                    ...
                    logger.info(f"✅ Iteration {i+1}: {bluesky_cmd}")
                else:
                    logger.error(f"❌ Iteration {i+1}: No JSON response")
                    break
                    
            except json.JSONDecodeError:
                logger.error(f"❌ Iteration {i+1}: JSON parse error")
                break
        
        success_rate = len(iteration_results) / max_iterations
        logger.info(f"🏁 Iterative test: {len(iteration_results)}/{max_iterations} successful ({success_rate:.1%})")
        
        return success_rate >= 0.8  # 80% success rate required
    
    def run_full_verification(self, scat_file: str = None) -> Dict[str, Any]:
        """Run complete verification suite."""
        logger.info("🚀 Starting COMPLETE Real LLM Verification")
        logger.info("=" * 60)
        ...
        if scat_file and os.path.exists(scat_file):
            ...
            logger.warning("⚠️ SCAT file not provided or not found - skipping SCAT chain test")
...
def main():
    ...
    print("🚀 Real LLM Communication Verification")
    print("=" * 50)
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.exists(scat_file):
            print(f"📂 SCAT file: {scat_file}")
        else:
            print(f"⚠️ SCAT file not found: {scat_file}")
            scat_file = None
    else:
        ...
        if os.path.exists(default_scat):
            ...
            print(f"📂 Using default SCAT file: {scat_file}")
        else:
            print("ℹ️ No SCAT file provided (SCAT chain test will be skipped)")
    ...
    try:
        ...
        print(f"⏱️ Total time: {results['total_time_sec']:.2f} seconds")
        print(f"🔄 LLM interactions: {len(results['debug_interactions'])}")
        
        if summary["overall_success"]:
            print("\n🎉 OVERALL RESULT: SUCCESS - Real LLM communication verified!")
            print("✅ All communication paths working:")
        ...
        print(f"\n💾 Detailed results: real_llm_verification_*.json")
        print(f"💾 Debug log: real_llm_verification.log")
